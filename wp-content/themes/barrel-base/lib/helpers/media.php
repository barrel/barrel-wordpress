...
function the_event_video($event_id) {
  $video = get_field('event_video_url', $event_id);
  $start_time = get_field('event_video_start_time', $event_id) ?: 0;
  $end_time = get_field('event_video_end_time', $event_id) ?: 'end';

  $attrs = ' data-start-time="'.$start_time.'" data-end-time="'.$end_time.'" muted';

  the_video_element($video, $attrs);
}
